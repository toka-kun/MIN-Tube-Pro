...
<head>
    ...
    <style>
        :root { 
            ...
            --ai-glow: 0 0 15px rgba(155, 114, 203, 0.4);
        }
        ...
        .ai-loading-bar { 
            ...
            position: relative; overflow: hidden; border-radius: 10px; margin: 12px 0;
        }
        .ai-loading-progress { 
        ...
        #aiJumpBtn:hover { transform: translateY(-3px); background: #252525; box-shadow: var(--ai-glow); }
        ...
        .description-box { background: var(--bg-secondary); border-radius: 12px; padding: 12px; font-size: 14px; margin-bottom: 24px; line-height: 1.6; }
        ...
        @media (max-width: 1000px) { 
            ...
            #aiJumpBtn { top: 60px; right: 10px; font-size: 12px; padding: 8px 15px; }
        }
    </style>
</head>
<body>
...
<div class="container">
    <div class="main-content">
        <div class="player-container">
            ...
            <div id="videoLoadingOverlay" style="position:absolute; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.8); display:none; flex-direction:column; align-items:center; justify-content:center; gap:15px; z-index:10;">
                <div class="spinner"></div>
                <div style="font-weight: bold;">読込中...</div>
            </div>
        </div>
        <h1 class="video-title">${videoData.videoTitle}</h1>
        <div class="owner-row">
            <div class="owner-info">
                <img src="${videoData.channelImage || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(videoData.channelName)}" alt="avatar">
                <div>
                    <div class="channel-name">${videoData.channelName}</div>
                    <div style="font-size:12px; color:var(--text-sub);">チャンネル登録者数 非公開</div>
                </div>
                <button id="subBtn" class="btn-sub" style="margin-left:12px;">チャンネル登録</button>
            </div>
            <div style="display:flex; gap:8px;">
                <button class="action-btn"><i class="far fa-thumbs-up"></i> ${videoData.likeCount || 0}</button>
        ...
        <div class="comments-section">
            ...
            ${commentsData.comments.slice(0, 10).map(c => `
                <div style="display:flex; gap:16px; margin-bottom:20px;">
                    <img src="${c.authorThumbnails?.[0]?.url || ''}" style="width:40px; height:40px; border-radius:50%;">
                    <div>
                        <div style="font-size:13px; font-weight:bold; margin-bottom:4px;">${c.author}</div>
    ...
    <div class="sidebar">
        <div id="aiRecContainer" class="ai-rec-container">
            <div class="ai-rec-header">
                ...
                <button class="ai-close-btn" onclick="dismissAI(event)"><i class="fas fa-times"></i></button>
            </div>
            <div id="aiRecContent">
                <div class="ai-loading-wrapper">
                    <div class="ai-loading-bar"><div class="ai-loading-progress"></div></div>
                    <div style="font-size:12px; color:var(--text-sub);">あなたの好みを学習しています...</div>
                </div>
            </div>
        </div>
...
<script>
    ...
    const VIDEO_TITLE = ${JSON.stringify(videoData.videoTitle)};
    const CHANNEL_NAME = ${JSON.stringify(videoData.channelName)};

    // 履歴保存
    function saveToHistory() {
        let history = JSON.parse(localStorage.getItem('yt_pro_history') || '[]');
        history = history.filter(v => v.id !== VIDEO_ID);
        history.unshift({ id: VIDEO_ID, title: VIDEO_TITLE, channel: CHANNEL_NAME, date: Date.now() });
        localStorage.setItem('yt_pro_history', JSON.stringify(history.slice(0, 30)));
    }
    ...
    async function getAIRecommendation() {
        ...
        const prompt = \`
以下の情報を基に、このユーザーが次に絶対に見たいと思うYouTube動画のタイトルを1つだけ推測して提案してください。
【視聴履歴】
\${history.map(h => "- " + h.title + " (" + h.channel + ")").join('\\n')}
...
\`;
        ...
        try {
            ...
            return await searchVideoOnBackend(suggestedTitle);
        } catch (err) {
    ...
    async function searchVideoOnBackend(query) {
        try {
            const res = await fetch(\`/api/recommendations?title=\${encodeURIComponent(query)}&channel=&id=ai_search\`);
            const data = await res.json();
    ...
    async function initAIUI() {
        if (localStorage.getItem('ai_feature_disabled') === 'true') return;

        const aiContainer = document.getElementById('aiRecContainer');
        const jumpBtn = document.getElementById('aiJumpBtn');
        const isFirstVisit = localStorage.getItem('ai_shown_on_sidebar') !== 'true';

        if (isFirstVisit) {
            aiContainer.style.display = 'block';
            localStorage.setItem('ai_shown_on_sidebar', 'true');
            loadAndDisplayAI();
        } else {
    ...
    // AIおすすめの取得と表示
    async function loadAndDisplayAI() {
        const content = document.getElementById('aiRecContent');
        const video = await getAIRecommendation();

        if (!video) {
            content.innerHTML = '<div style="font-size:12px; color:var(--text-sub); text-align:center; padding:10px;">現在おすすめを生成できません。</div>';
            return;
        }

        content.innerHTML = \`
            <a href="/video/\${video.id}" class="rec-item" style="margin-bottom:0; padding:8px; border-radius:8px; background:rgba(255,255,255,0.03);">
                <div class="rec-thumb">
                    <img src="https://i.ytimg.com/vi/\${video.id}/mqdefault.jpg">
                    <div style="position:absolute; bottom:4px; right:4px; background:rgba(155, 114, 203, 0.9); font-size:10px; padding:2px 6px; border-radius:4px; font-weight:bold;">AI推奨</div>
                </div>
                <div class="rec-info">
                    <div class="rec-title">\${video.title}</div>
                    <div class="rec-meta">\${video.channelTitle}</div>
                    <div class="rec-meta" style="margin-top:4px; color:#9b72cb; font-size:11px; font-weight:bold;"><i class="fas fa-magic"></i> パーソナライズ済み</div>
                </div>
            </a>
        \`;
    }
    ...
    function triggerAIRecommendationUI() {
        const container = document.getElementById('aiRecContainer');
        ...
        document.getElementById('aiRecContent').innerHTML = \`
            <div class="ai-loading-wrapper">
                <div class="ai-loading-bar"><div class="ai-loading-progress"></div></div>
                <div style="font-size:12px; color:var(--text-sub);">最新の履歴から解析中...</div>
            </div>\`;
        loadAndDisplayAI();
    }

    // 非表示設定
    function dismissAI(e) {
        e.stopPropagation();
        if (confirm("AIおすすめ機能を無効にしますか？（この設定は保存されます）")) {
            localStorage.setItem('ai_feature_disabled', 'true');
    ...
    async function changeServer(mode) {
        ...
        try {
            // ...サーバー選択ロジック（既存のものを維持）...
            let url = mode === 'googlevideo' ? \`https://www.youtube-nocookie.com/embed/\${VIDEO_ID}?autoplay=1\` : \`/pro-stream/\${VIDEO_ID}\`;
            player.innerHTML = \`<iframe src="\${url}" frameborder="0" allowfullscreen style="width:100%; height:100%;"></iframe>\`;
        } catch(e) { console.error(e); }
        finally { overlay.style.display = 'none'; }
    }

    async function loadNormalRecs() {
        const res = await fetch(\`/api/recommendations?title=\${encodeURIComponent(VIDEO_TITLE)}&channel=\${encodeURIComponent(CHANNEL_NAME)}&id=\${VIDEO_ID}\`);
        const data = await res.json();
    ...
    window.onload = () => {
        ...
        loadNormalRecs();
        initAIUI();
        changeServer(localStorage.getItem('playbackMode') || 'googlevideo');
    };
</script>
</body>
</html>
